...
%% just as usual
\author{P. M. E. Altham and Robin K. S. Hankin\\University of Cambridge}
...
\Abstract{Recent work by~\citeauthor{lin2009} proposed a Bayesian
  ...
  This vignette is based on a manuscript submitted to the Journal of
  the Royal Statistical Society Series C, as correspondence.
...
is the appropriate hyperdirichlet distribution.  The hyperdirichlet
...
hyperdirichlet \proglang{R} package~\citep{hankin2009}.

<<generate_data,echo=FALSE,print=FALSE,cache=TRUE>>=
if(calculate_from_scratch){
  number <- 10000
  set.seed(0)
  d <- rhyperdirichlet(b, n=number)
...
f3 <- function(x){x[2]<x[3]}
mlb.f3 <- max.like(b,disallowed=f3)
@ 
...
Our analysis has added to the techniques which practising statisticians may
bring to bear on the analysis of this type of \twobytwo\ table, and we hope to
stimulate interest in the aylmer and hyperdirichlet \proglang{R} packages.
