...
\Abstract{This vignette is based on~\citet{hankin2010}.  It
  ...
  For reasons of performance, some of the more computationally
  expensive results are pre-loaded.  To calculate them from scratch,
  change ``\code{calc\_from\_scratch <- TRUE}'' to
  ``\code{calc\_from\_scratch <- FALSE}'' in chunk \code{time\_saver}.
